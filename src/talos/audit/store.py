...
from talos.audit.models import AuditActionRecord, AuditExportBundle
from talos.audit.redaction import AuditRedactionError, redact_json
...
def default_audit_store() -> AuditStore:
    override = os.environ.get("TALOS_AUDIT_DIR")
    root = (
        Path(override)
        if override
        else user_data_path("talos-freecad-mcp", appauthor=False) / "audit"
    )
