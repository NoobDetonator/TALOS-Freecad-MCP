...
from talos.bridge.transport import BridgeEndpoint, create_session_token
...
def test_default_store_uses_explicit_runtime_directory(
    ...
    monkeypatch.setenv("TALOS_RUNTIME_DIR", str(runtime))
    monkeypatch.delenv("TALOS_PROJECT_ROOT", raising=False)
...
def test_default_store_uses_the_user_runtime_directory(
    ...
    monkeypatch.delenv("TALOS_RUNTIME_DIR", raising=False)
    monkeypatch.setattr(
        "talos.bridge.session.user_runtime_dir",
...
def test_default_store_does_not_put_session_token_in_project(
    ...
    monkeypatch.delenv("TALOS_RUNTIME_DIR", raising=False)
    monkeypatch.setenv("TALOS_PROJECT_ROOT", str(project_root))
    monkeypatch.setattr(
        "talos.bridge.session.user_runtime_dir",
